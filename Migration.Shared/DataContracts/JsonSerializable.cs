...
{
    public abstract class JsonSerializable //We introduce this type so that we dont have to expose the Newtonsoft type publically.
    {
        ...
        {
            JustPocoSerialization = int.TryParse(
                string.IsNullOrWhiteSpace(Environment.GetEnvironmentVariable(POCOSerializationOnly)) ?
                    "0" :
                    Environment.GetEnvironmentVariable(POCOSerializationOnly), out int result) && result == 1;
        }
        ...
        public virtual void LoadFrom(JsonReader reader, JsonSerializerSettings serializerSettings)
        {
            ...
            SetupJsonReader(reader, serializerSettings);
        ...
        internal T GetValueByPath<T>(string[] fieldNames, T defaultValue)
        {
            ...
            if (this.propertyBag != null)
            {
                ...
                for (int i = 1; i < fieldNames.Length; ++i)
                {
                    ...
                    token = !(token is JObject) ? null : token[fieldNames[i]];
                }
        ...
        internal void SetValueByPath<T>(string[] fieldNames, T value)
        {
            ...
            if (value == null && token is JObject tokenAsJObject)
            {
                tokenAsJObject.Remove(fieldNames[^1]);
            }
            else
            {
                token[fieldNames[^1]] = value == null ? null : JToken.FromObject(value);
            }
        }
        ...
        internal TSerializable GetObject<TSerializable>(string propertyName, bool returnEmptyObject = false) where TSerializable : JsonSerializable, new()
        {
            if (this.propertyBag != null)
            {
                ...
                if (token != null && (returnEmptyObject || token.HasValues))
                {
                    ...
                    {
                        propertyBag = JObject.FromObject(token)
                    };
                    return result;
                }
            }
        ...
        internal void SetObject<TSerializable>(string propertyName, TSerializable value) where TSerializable : JsonSerializable
        {
            ...
            this.propertyBag[propertyName] = value?.propertyBag;
        }
            ...
            if (this.propertyBag != null)
            {
                ...
                if (token != null)
                {
                    ...
                    foreach (JObject childObject in jobjectCollection)
                    {
                        ...
                        if (IsPublicResource(typeof(TSerializable)))
                        {
                            Resource resource = result as Resource;
                            resource.AltLink = GeneratePathForNameBased(resourceType, ownerName, resource.Id);
                        }
        ...
        private static string GeneratePathForNameBased(Type resourceType, string resourceOwnerFullName, string resourceName)
        {
            ...
            else if (typeof(DocumentMetadata).IsAssignableFrom(resourceType))
            {
        ...
        {
            SetupJsonReader(jsonReader, settings);
